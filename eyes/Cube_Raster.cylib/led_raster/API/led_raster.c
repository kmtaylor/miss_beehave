...
CY_ISR(`$INSTANCE_NAME`_isr) {
    ...
    for (i = 0; i < RGB_SIZE; i++) { 
        ...
        REG_WRITE(LED_RASTER_RGB_DATA_7, rgb_d[7][rgb_i + i]);
        REG_WRITE(LED_RASTER_RGB_DATA_8, rgb_d[8][rgb_i + i]);
        REG_WRITE(LED_RASTER_RGB_DATA_9, rgb_d[9][rgb_i + i]);
    }
    rgb_i += RGB_SIZE;
...
void `$INSTANCE_NAME`_SetupDMA(uint16 chain_length, uint8 **rgb, uint8 *offset,
		uint8 *count) {
    rgb_d = rgb;
    offset_d = offset;
    count_d = count;
...
#define DMA_TD_INIT(_i, _size, _data, _reg) \
    ...
    DMA_TD_INIT(7, RGB_SIZE * chain_length, rgb_d[7], LED_RASTER_RGB_DATA_7);
    DMA_TD_INIT(8, RGB_SIZE * chain_length, rgb_d[8], LED_RASTER_RGB_DATA_8);
    DMA_TD_INIT(9, RGB_SIZE * chain_length, rgb_d[9], LED_RASTER_RGB_DATA_9);
